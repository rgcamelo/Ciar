...
<script>
        ...
        $(document).ready(inicio);
            ...
            function inicio()
            {
                $("#boton").click(function(){
                    if( validar() == false){
                        ...
                        $("#loginform").attr("action","{{ url('/reseñasCriticas') }}");
                    }
                });
        
                $("#boton2").click(function() {
                    limpiar();
                    $("#loginform").attr("action","{{ url('/guardarreseñasCriticas') }}");
                })
            }
            function limpiar() { 
                $('#iconotitulo').remove();
            ...
            function validar(){
                        ...
                        var noautores = $("#noautores_reseña").val();
                        ...
                        if( noautores == null || noautores.length == 0 || /^\s+$/.test(noautores)){
                            ...
                            $("#noautores_reseña").attr("placeholder","Ingrese el numero de autores");
                            valido=false;
                        }else{
                            $('#icononoautores').remove();
            ...
            <div id="loginbox" style="margin-top:10px;" class="mainbox col-md-8 col-md-offset-2 col-sm-8 col-sm-offset-2">                    
                <div class="panel panel-success" >
                        <div class="panel-heading">
                            <div class="panel-title">Datos de la Reseña Critica</div>
                        ...
                        <div style="padding-top:30px" class="panel-body" >
                            ...
                            <form id="loginform" method="post" action="" class="form-horizontal" role="form" enctype="multipart/form-data">
                                ...
                                <input id="noautores_reseña" type="number" class="form-control"  name="noautores" min="1" value="{{old('noautores')}}">
                                
                                </div>
                                
                                    <div style="margin-top:10px" class="form-group">
                                        <div>
                                                @if (isset(auth()->user()->Docente()->Productividad()->reseñas))
                                                @if (auth()->user()->Docente()->Productividad()->reseñas <5 or auth()->user()->Docente()->Productividad()->reseñas <5)
                                                <center>
